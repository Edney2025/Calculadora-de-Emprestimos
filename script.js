document.addEventListener('DOMContentLoaded', () => {
    ...
    function atualizarEstadoBotoes() {
        if (estado.etapaAtual < 5) getElem('btn-avancar').disabled = !validarEtapa(estado.etapaAtual);
        else getElem('btn-avancar').disabled = true;
    }
    ...
    function gerarOpcoesDePagamento() {
        ...
        for (let p = 3; p <= 36; p += 3) {
            ...
            div.innerHTML = `<strong>${p}x</strong> de ${formatarMoeda(opcao.valorParcela)}<br><small>Taxa: ${(opcao.taxaJuros * 100).toFixed(2)}% a.m.</small>`;
    ...
    function configurarEtapa4() {
        ...
        container.innerHTML = `
            <div class="summary-grid">
                ...
                <div class="summary-card"><span>📊 Faixa de Empréstimo</span><strong>${faixaSelecionada}</strong></div>
                ...
                <div class="summary-card"><span>📈 Taxa de Juros</span><strong>${(op.taxaJuros * 100).toFixed(2)}% a.m.</strong></div>
            </div>
    ...
    function compartilharWhatsApp() {
        ...
        const tabela = gerarTabelaAmortizacao(o);
        const faixaSelecionada = `R$ ${estado.faixaPreco.min.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} - R$ ${estado.faixaPreco.max.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
        let parcelasTexto = tabela.map(r => `Parcela ${r.parcelaNum}: ${formatarMoeda(o.valorParcela)} - Vencimento: ${r.vencimento}`).join('\n');
        let txt = `Prezado(a) ${CONFIG.credorNome},\n\n` +
                 ...
                 `Taxa de Juros: ${(o.taxaJuros * 100).toFixed(2)}% a.m.\n` +
                 ...
                 `Aguardamos a análise inicial em até 48 horas e a aprovação final entre 10 e 90 dias, conforme o valor solicitado.\n\n` +
                 `Atenciosamente,\n${d.nomeCompletoCliente}`;
        const mensagemAlerta = `Sua solicitação foi recebida com sucesso!\n\n` +
                              `A primeira etapa de avaliação ocorrerá em até 48 horas.\n` +
                              `O prazo para a aprovação final e liberação do valor é de 10 a 90 dias, a depender do montante solicitado e da análise de crédito. Este período é necessário para realizarmos todas as verificações com segurança.\n\n` +
